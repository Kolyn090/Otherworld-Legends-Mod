...
public class FixSpriteDumpTexturePathID : EditorWindow
{
    private string _owningFolderPath = "";
    private string _replacePathID = "";

    [MenuItem("Tools/08 Fix Texture Path ID", priority = 8)]
    public static void ShowWindow()
    {
        GetWindow<FixSpriteDumpTexturePathID>("Fix Sprite Dump Texture Path ID");
    }
    ...
    private void OnGUI()
    {
        ...
        string messageEn = "Usage: replace Sprite Texture Path ID. The purpose of this tool is to replace " +
                            "My Sprite Path ID to Source Sprite Path ID. Warning: this tools does not fit " +
                            "Dump files that use SpriteAtlas; If used, run '09 Fix Atlas Path ID'.";
        string messageZh = "用途：替换图像的材质路径ID。该工具主要使用目的为替换路径ID成源游戏的路径ID。" +
                            "注意：该工具不适用于有使用图集的文件，如果有使用图集，运行 '09 Fix Atlas Path ID'。";
    ...
    private void Fix()
    {
        ...
        foreach (string jsonFile in jsonFiles)
        {
            ...
            {
                spriteJson["m_RD"]["texture"]["m_PathID"] = long.Parse(_replacePathID);
            }
